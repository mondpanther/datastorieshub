...
```{r}
  ...
  data= data %>% mutate(HE= education=="some col" | education=="col+" ,
                        employed2=employed=="Yes")

  mod1 <- lm(employed2~black+HE,data)
  summary(mod1)
  mod2 <- lm(employed2~black*HE,data)
  summary(mod2)
```
...
Is the discriminations significant for educated people

```{r}
library(car)
linearHypothesis(mod2,"black:HETRUE+black=0")
...
```{r}
  lm(wage~educ+female,data)%>%summary()
