...
# Getting data
(note that starting a line with `#` creates a heading. Starting with `##` is a sub heading. Also note that the mening of `#` is different within a code chunk. There it represents a comment; i.e. everything that comes after `#` on the same line of code is not taken into account by the R interpreter who considers it commentary for humans rather than code. Note that we sometimes can use this as well to `comment out` parts of the code that we don't want to run at the moment but might need later again.)
...
covid_july=covid %>% filter(as.character(date)=="2020-07-31") 
...
p=ggplot(covid_julyb, aes(x=density, y=casesOpop000 ))+ 
       geom_point() 
p
