...
```{r, echo = TRUE}  
  library(foreign)
  data <- read.dta("https://www.dropbox.com/sh/rqmo1hvij1veff0/AADZ4MZhDDk9R8sFSjBvmcRma/WAGE1.DTA?dl=1", header=TRUE) 
```
...
Can you reject the hypothesis? (YES/NO) `r fitb("YES")`
...
Can you reject the hypothesis? (YES/NO) `r fitb("NO")`
...
```{r, echo = TRUE}  
  library(foreign)
  data <- read.dta("https://www.dropbox.com/sh/rqmo1hvij1veff0/AACNkMy47ilXAMh3nmiIs_Bqa/auto.dta?dl=1",header=TRUE)
```

### Part (`r part()`)
Draw a scatterplot of price versus weight. 
Which cars, in terms of weight, tend to be more expensive? (HEAVIER/LIGHTER)`r fitb("HEAVIER")`
...
Do you think this number is very meaningful? (YES/NO) `r fitb("NO")` Why or why not?
...
Otherwise take the same steps as in the previous Monte Carlo exercise, using the the same true model $Y=2+0.5 X +\epsilon$[PS NOTE: I think the question doesn't really work like that, need to edit or provide hints?...]
