...
Can you reject the hypothesis? `r mcq(c("No", answer = "Yes"))`
...
```{r, echo = TRUE}
  library(ggplot2)
  ggplot(data, aes(y=price, x=weight)) + geom_point()
```
...
What is the predicted price of a car weighing 4,000lb?  `r fitb(8169.543)`
...
i.e. with `mod$coefficients[[1]]` and `mod$coefficients[[2]]` you can extract the $\beta$ coefficients from the regression stored in the variable `mod`.
