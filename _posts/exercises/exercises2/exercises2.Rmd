...
We see that the dataframe contains daily COVID case and death numbers for every us state.
...
Compute the total number of deaths and cases by July 31st 2020. Try to work this out using online tips at first. If you spend to much time working it out check the solution.


`r hide("Answer")`

With `filter()` we can select specific rows of a dataframe. With `group_by()` you can group the rows of a dataframe into groups defined by one or indeed several categorical variables. 
Note that the date variable is a factor variable. You can see this in the `Global Environment` tab:
...
covid_sum=covid %>% dplyr::filter(as.character(date)<"2020-08-01") %>%
          group_by(state) %>% 
