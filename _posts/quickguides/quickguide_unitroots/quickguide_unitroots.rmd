...
```
If we run a regression of `yyy` on `x100` we get a wildly biased estimate. If we run a regression in first differences we get something fairly close to the true value.
...
Turns out the `z.diff.lag1` is not signficiant either. So maybe back to `lags=0`?
