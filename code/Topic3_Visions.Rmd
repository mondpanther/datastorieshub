...
p=ggplot(states_mapPdata, aes(long, lat, group = group))+
  geom_polygon(aes(fill = hoaxsh),color="white")+theme_void()+
...
```{r}
centroid <- aggregate(cbind(long,lat) ~ region, data=states_mapPdata, FUN=mean)

p+geom_text(data = centroid, mapping = aes(x=long, y=lat, label=region)) 


testtext <- ggplot() +
            geom_polygon(data = df, mapping = aes(x=long, y=lat, group = group, fill=location)) +
            geom_text(data = centroid, mapping = aes(x=long, y=lat, label=region)) +
