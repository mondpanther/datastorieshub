...
ldppweek=wei %>% mutate( Fweek=lead(week) ) %>% merge(ldpp) %>% filter(Date>week & Date<Fweek) %>% 
         group_by(week) %>% 
...
df=wei %>%  left_join(covidus, by="week") %>%  
   ...
   left_join(ldppweek,by="week") %>% mutate(lockshare=replace(lockshare,is.na(lockshare),0))
...
lm(Dlnindex~Dcases+t+lockshare,df) %>% summary()

lm(Dlnindex~lag(Dlnindex)+Dcases+lag(Dcases)+t+lockshare+lag(lockshare),df) %>% summary()  
