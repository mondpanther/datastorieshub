...
```{r,message=FALSE}
  ...
  ur.df(df$cases,type="trend",lags=4) %>% summary()
  ur.df(df$lnindex,type="trend",lags=4) %>% summary()
...
```{r}  
  ...
  ur.df(diff(df$cases,1),type="trend",lags=4) %>% summary()

  ur.df(diff(df$lnindex,1),type="trend",lags=4) %>% summary()
  ur.df(diff(df$lnindex,1),type="none",lags=4) %>% summary()
