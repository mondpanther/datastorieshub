...
There are clearly further concerns to explore. For instance with the data we already have we can easily include a further control variable: the number of tweets per capita. This could be another control for confounding influences such as education for instance, which might only impefectly be captured by population density; e.g. states withmore educted people will have higher internet (and twitter) usage. The resulting regression is as follows:
